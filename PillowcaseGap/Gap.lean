...
metric-space level, exactly the level of the positivity lemma it
certifies; the length-space structure of the setting enters nowhere
in the lemma. Compact connected Riemannian manifolds with continuous
metric, carrying their length distance, are instances of the setting
by an in-document separation estimate, and `ι` below is the injective
inclusion of the pillowcase carrying the induced distance.
